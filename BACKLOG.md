...
- [x] **Tooltip audit — all charts** — review every chart's tooltip for (1) correct mode (`nearest`/`index` as appropriate), (2) content that goes beyond what's already visible on the chart (no redundant labels), and (3) enriched context where the underlying data supports it (e.g., breakdown lines, state/venue, comparison to average). Model: the VIP scatter tooltip refactor. Charts to audit: all t1, t2, t3, t4 charts. Affects: `src/charts.js` (tooltip callbacks throughout).
